...
(function (root, plugin) {
	...
	/**
        ...
        _drawHotline: function (ctx) {
            ...
            var i, j, dataLength, path, pathLength, point;
            ...
            for (i = 0, dataLength = this._data.length; i < dataLength; i++) {
                ...
                for (j = 0, pathLength = path.length; j < pathLength; j++) {
                    ...
                    var gradient = ctx.createRadialGradient(point.x, point.y, 0, point.x, point.y, this._weight / 2);
                    var gradientStartRGB = this.getRGBForValue(point.z);
                    var gradientEndRGB = this.getRGBForValue(point.z);
                    ...
                    ctx.fillStyle = gradient;
                    ctx.beginPath();
                    ctx.arc(point.x, point.y, this._weight / 0.8, 0, 2 * Math.PI);
                    ctx.closePath();
                    ctx.fill();
